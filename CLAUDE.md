...
- **O tombo é do corpo, não do golpe** (`Sounds/EnemyFall`, 0,7): o grunhido do Pia Loco sai quando ele **chega ao chão** — poise estourado, knockdown forçado do boliche, aterrissagem do arremesso ou morte —, e não a cada golpe recebido. Um zumbi que geme em todo soco geme várias vezes por segundo e vira ruído; derrubar é o **pagamento** da corrente, e era o único momento do combate sem confirmação sonora. O lançamento em si é mudo de propósito: o corpo grunhe quando **aterrissa**, que é o que se vê na tela. O volume é o mesmo do impacto (0,7, e não abaixo como o swing): o soco e o corpo caindo são um evento só para o ouvido, e um grunhido por baixo leria como cauda do soco em vez do pagamento.
- **Um grunhido por corpo, e não um por frame** — a exceção deliberada à regra do `PlayHitSound`. Aquela existe contra **duas cópias do mesmo evento** (um golpe que pega dois mooks é *um* golpe, e dois one-shots iguais no mesmo frame só somam amplitude); aqui são **dois corpos**, duas vozes, e calar um deixaria metade da pilha muda. Por isso a fiação é **por inimigo** e não uma chamada no laço de combate. Quem sinaliza é `Fighter.OnFell` (`Action`, sem argumento: o assinante já sabe de quem é o corpo, e tombo e morte soam igual), disparado em dois pontos — no fim de `TakeDamage` e no fim de `Land()`. Não há rastreio de transição porque o **guarda no topo de `TakeDamage`** já devolve cedo para quem estava `Dead`/`KnockedDown`/`Thrown`: terminar o método no chão *é* ter caído nessa chamada. O predicado é `IsOnTheFloor`, e **não** `!IsAlive`, que também pega `Thrown` — um corpo no ar ainda não bateu em nada.
- **Quem assina é o berço, não o spawn**: `EnemyFactory` recebe um `Action<PiaLocoEnemy> onSpawned` e o chama em **`Create`**, não dentro do `BuildPiaLoco` — `Create` é a porta por onde todo inimigo sai, inclusive um tipo registrado depois via `Register`, que nasceria mudo se o gancho ficasse no builder embutido. A arena passa o seu `WireEnemy` ali, do mesmo jeito que assina a Sofia no construtor; nada a desassinar, porque o delegate aponta **do** inimigo **para** a arena, que o larga junto com a fase. **A Sofia fica de fora de propósito**, simétrico à multidão ficar de fora do whoosh: a voz é do zumbi, o golpe é dela.
